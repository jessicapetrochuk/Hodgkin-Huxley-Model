...
def test_transition_rate_alpha_n():
    V = 100
    trans_rate = 'alpha_n'
    assert transition_rate(V, trans_rate) == (0.01 * (10 - 100)) / (math.exp((10 - 100) / 10) - 1)

def test_transition_rate_beta_n():
    V = 100
    trans_rate = 'beta_n'
    assert transition_rate(V, trans_rate) == 0.125 * math.exp(-100 / 80)

def test_transition_rate_alpha_m():
    V = 100
    trans_rate = 'alpha_m'
    assert transition_rate(V, trans_rate) == (0.1 * (25 - 100)) / (math.exp((25 - 100) / 10) - 1)

def test_transition_rate_beta_m():
    V = 100
    trans_rate = 'beta_m'
    assert transition_rate(V, trans_rate) == 4 * math.exp(-100 / 18)

def test_transition_rate_alpha_h():
    V = 100
    trans_rate = 'alpha_h'
    assert transition_rate(V, trans_rate) == 0.07 * math.exp(-100 / 20)

def test_transition_rate_beta_h():
    V = 100
    trans_rate = 'beta_h'
    assert transition_rate(V, trans_rate) == 1 / (math.exp((30 - 100) / 10) + 1)
